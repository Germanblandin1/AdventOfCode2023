...
func ContarCaminos(i, j, dir, costo int) int {
	//fmt.Printf("i=%d j=%d dir=%d costo=%d\n", i, j, dir, costo)
	count := 0
	if !visited[i][j] {
		count++
		visited[i][j] = true
	...
	for k := -1; k <= 1; k++ {
		...
		//fmt.Printf("i=%d j=%d dir=%d costo=%d dist=%d\n", ni, nj, newDir, costo-sum, dist[newDir][ni][nj])
		if costo-sum == dist[newDir][ni][nj] {
			count += ContarCaminos(ni, nj, newDir, costo-sum)
		}
	}
	return count
}
...
func main() {
	...
	for i := 0; i < 4; i++ {
		if costoTotal == dist[i][finI][finJ] {
			total += uint64(ContarCaminos(finI, finJ, i, costoTotal))
		}
	}
